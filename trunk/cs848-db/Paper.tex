...
The main idea behind using materialized views is to rewrite queries using these views so that they run more efficiently. Given the semi-structured nature of
...
problem~\cite{tang2009materialized}. Therefore, the choice of which views to materialize becomes more complicated in XML database. That is, of course, besides
the difficulty of selecting which views to use for rewriting. However, this problem is not within the scope this survey. Since XPath query processing is
expensive, materialized views come in handy in XML databases. Balmin et al.~\cite{balmin2004framework} propose a framework for
...
performance because queries tend to access less data. Such inlining is similar to horizontal partitioning~\cite{ramanath2003searching}.
