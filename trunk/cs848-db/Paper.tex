...
Section~\ref{SEC-OPEN} mentions some of the
...
opposed to examining all the 1000 records. Horizontal partitioning is more widely used, and is implemented in all current DBMSes. Table~\ref{tab:horpartition}
...
\hline
\textbf{Benefits} & \multicolumn{4}{c}{Partition elimination, Improved administration, Fast roll-in/out, and Implicit clustering }
\\
\hline
...
Clustering reflects how the records are physically located together on disk. This is very important since sequential I/O access is much cheaper than random
access. Therefore, records that are likely to be queried together should be stored physically together to decrease the number of random I/O accesses.
MOREEEEEEEEEEEEEEEE
...
throughout the research community. These challenges will be discussed throughout this section. Table~\ref{tab:mmdbsumm} summarizes
...
\item CSS Tree
                                   \end{itemize}\\
\hline
Materialized Views & \begin{itemize} \item Not highly utilized since processing and memory access is cheap \end{itemize}& N/A\\
\hline
...
\item Vertical Partitioning
                                                                                           \end{itemize}\\
\hline
Clustering & \begin{itemize} \item Not needed since random or dispersed main memory access is not more expensive than sequential access \end{itemize}
& N/A
...
\hline
 \textbf{Index Structure} & \textbf{Features} & \textbf{Cache Consciousness}\\
\hline\hline
B Trees &
...
\item Stores child nodes in an array. Stores pointer to first child only and uses index calculation to get the rest
             \end{itemize} &
\begin{itemize}
 \item Good cache performance due to improved locality \& lower tree height (because of larger node size)
\end{itemize}\\
\hline
...
A second question is how to characterize
scenarios or classes of workloads for which constrained dynamic
physical designs will be beneficial.

Similarly, IBM~\cite{} suggested a similar technique for their database with some changes such as selecting the order of
