...
\title{A Survey of Physical Design Techniques of Information Systems -TEST }
...
\begin{figure}[!t]
\centering
\includegraphics[width=9cm]{Ttree.png}
...
There are two ways to store XML documents in a database. The first is to have the database internally translate the XML document into relational tables (XML-enabled databases). The second is to have data structures that can persistently store XML documents (native XML databases). eXist~\cite{meier2009exist} and TIMBER~\cite{jagadish2002timber} are two of the famous research efforts to design a native XML database.
