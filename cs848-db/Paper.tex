...
Table~\ref{tab:mmdbindexsumm} summarizes the features of the different index structures discussed in this section with respect to MMDB.
...
\hline
 & \textbf{Features suitable for MMDB} & \textbf{Cache Consciousness}\\
\hline\hline
B Trees &
...
more suitable since there is no gain from keeping all the datGood storage utilizationa in the leaves. In a main memory system, this would only waste space
without improving
...
more than more comparison leading to better cache utilization~\cite{rao1999cache}. Therefore, B and B+ Trees in general have reasonable cache behavior.
