...
Relational Database Systems have been the most popular type of Information Systems for decades. When we think of designing a database systems, we usually think
of which data we need to keep track of and how will we properly organize it into meaningful tables. However, this is not the only phase in database design.
All of our designed tables need to be physically stored on disk. How these tables will be stored and accessed greatly affects the database performance. Physical
...
systems. This includes explaining how alterations or additions are made to data structures to fit the specific needs of every system. For a complete reference
on the details of the data structures used in physical database design, please see (CITE HERE). Using the different physical design methods used in disk based
relational database systems, we explore how these methods apply to the other systems and where the differences and similarities lie. 


The rest of the survey is organized as follows. Section~\ref{SEC-ELEMENTS} first explains different elements of physical database design based on traditional
disk-based databases. Section~\ref{SEC-DIFFSYS} then discusses how each of the three other systems uses these elements, as applicable, to improve its
efficiency.Sections~\ref{SEC-MMDB},~\ref{SEC-XML},~\ref{SEC-WAREHOUSES}, discuss Main Memory Databases, XML Databases, and Data Warehouses
...
Section~\ref{SEC-AUTO} then presents how the choice of the different physical design techniques can be automated. Section~\ref{SEC-OPEN} mentions some of the
open problems of physical database design, and suggests possible solutions. Finally Section~\ref{SEC-CONCL} summarizes and concludes this survey.

\section{Elements of Physical Database Design}
\label{SEC-ELEMENTS}


Relational Databases were first introduced by Codd~\cite{codd1970relational}. Disk based Relational Database Systems (RDBMS) are the most common and popular
...
instead of havinsummarizesg one value per node, a B-tee can have many values per node~\cite{comer1979ubiquitous} as shown in Figure (PUT A FIGURE FOR A B TREE
HERE).
...
mention clustered vs. unclustered ?
...
Put table from database about the diff horizontal partitioning 
...
talk about Tamino (http://www.rpbourret.com/xml/XMLAndDatabases.htm)~\cite{bourret2003xml}
...
administrators. Accordingly, researchers have been inclined to find ways to automate these decisions. There were many research efforts done in this
area(E.g.~\cite{finkelstein1988physical, hammer76selec, frank1992selec}), but most of these efforts suffered from either examining too many configurations, not
accounting for the query workload, or not considering if the query optimizer will benefit from the proposed configurations or not. Chaudhuri and
Narasayya~\cite{chadhuri1997selec} were the first to propose an efficient automatic index selection tool that is dependent on the query workload and uses the
query optimizer to evaluate the proposed configuration. They find the best configuration for the workload by examining each query individually to produce
single-column indexes then combining these configurations. They then run a greedy algorithm to select the best configuration with the least cost (as determined
by the query optimizer). They then try to further optimize the configuration by looking for possible multicolumn indexes as a combination of the single-column
indexes already found. One of the main strengths of this technique was that all these indexes were virtually created such that the query optimizer could use
them, but without having to incur the cost of physically creating them. This solution was part of the research at Microsoft and was tested in Microsoft SQL
Server 7.0.

Similarly, IBM~\cite{schiefer1999db2, valentin2000select} suggested a similar technique for their database with some changes such as selecting the order of
columns within an index based on the order in which the query optimizer evaluates the predicates. 
